...
public class ReclamationController implements Initializable {
    ...
    private void loadReclamations() {
        ...
        totalPages = (int) Math.ceil((double) allReclamations.size() / pageSize);
        if (totalPages == 0) totalPages = 1;
        if (currentPage > totalPages) currentPage = totalPages;
        int fromIndex = (currentPage - 1) * pageSize;
        int toIndex = Math.min(fromIndex + pageSize, allReclamations.size());
        ObservableList<Reclamation> pageList = FXCollections.observableArrayList();
        if (!allReclamations.isEmpty() && fromIndex < allReclamations.size()) {
            pageList.addAll(allReclamations.subList(fromIndex, toIndex));
        }
        displayReclamations(pageList);
